...
export const PhotoInput: React.FC<PhotoInputProps> = ({ onImageSelect, t }) => {
  ...
  return (
    <div className="relative flex flex-col w-full h-full pt-20 overflow-y-auto custom-scrollbar animate-fade-in">
      ...
      {/* Main Content Area - Flex grow ensures it takes space but allows footer to push down */}
      <div className="relative z-10 flex-1 flex flex-col items-center justify-center p-6 min-h-[450px]">
        {/* Main Content Card */}
        <div className="max-w-md w-full bg-slate-800/60 backdrop-blur-xl border border-slate-700/50 rounded-3xl p-8 shadow-2xl">
      ...
      {/* Footer / Credits / Disclaimer - Responsive Layout (Column on Mobile, Row on Desktop) */}
      <div className="relative z-20 w-full p-6 mt-auto text-center pointer-events-auto bg-gradient-to-t from-slate-900/90 via-slate-900/50 to-transparent flex flex-col md:flex-row items-center justify-center gap-2 md:gap-3">
        <p className="text-[10px] sm:text-xs text-slate-400/60 max-w-xs md:max-w-none">
           {t.disclaimer}
        </p>
        
        {/* Divider dot visible only on desktop */}
        <span className="hidden md:inline text-slate-600/50 text-[10px]">•</span>
