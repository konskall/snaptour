...
import { Play, Pause, ExternalLink, MapPin, Sparkles, Loader2, Share2, Check, MessageCircle, Map as MapIcon, Compass, ChevronDown, ChevronUp } from 'lucide-react';
...
export const TourCard: React.FC<TourCardProps> = ({ result, onReset, onChat, t, isAudioLoading = false, langCode = 'en' }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col justify-end sm:justify-center p-0 sm:p-6 pointer-events-none">
      ...
      {/* Card Content */}
      <div className="relative pointer-events-auto w-full max-w-2xl mx-auto bg-slate-900/90 backdrop-blur-xl border-t sm:border border-slate-700/50 sm:rounded-3xl shadow-2xl overflow-hidden max-h-[80dvh] sm:max-h-[85vh] flex flex-col animate-slide-up">
        ...
        <div className="p-6 pb-2 flex-shrink-0">
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <h1 className="text-xl sm:text-3xl font-bold text-white leading-tight line-clamp-2">{result.landmarkName}</h1>
            </div>
            ...
            <div className="flex gap-3 flex-shrink-0">
              <button
                onClick={onChat}
                className="w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-slate-700/80 hover:bg-slate-600 text-indigo-300 flex items-center justify-center border border-slate-600 transition-all hover:scale-105"
                title={t.askGuide}
              >
              ...
              <div className="flex-shrink-0 w-10 h-10 sm:w-12 sm:h-12">
                {isAudioLoading ? (
                   <div className="w-full h-full rounded-full bg-indigo-600/50 border border-indigo-500/50 flex items-center justify-center">
                     <Loader2 size={20} className="text-indigo-200 animate-spin" />
                   </div>
                ) : result.audioBuffer ? (
                  <button
                    onClick={toggleAudio}
                    className="w-full h-full rounded-full bg-indigo-600 hover:bg-indigo-500 text-white flex items-center justify-center shadow-lg shadow-indigo-500/30 transition-all hover:scale-105"
                  >
        ...
        {/* Scrollable Content - Added flex-1 min-h-0 to ensure it shrinks properly */}
        <div className="flex-1 min-h-0 p-6 pt-2 overflow-y-auto custom-scrollbar space-y-6">
        ...
        {/* Footer Actions - Added pb-8 for iPhone safe area */}
        <div className="flex-shrink-0 p-4 pb-8 sm:pb-4 border-t border-slate-800 bg-slate-900/50 backdrop-blur-sm flex gap-3">
