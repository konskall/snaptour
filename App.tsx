...
const App: React.FC = () => {
  ...
  // PERSISTENCE: Check for stored user on mount
  useEffect(() => {
    const storedUser = localStorage.getItem('snaptour_user');
    if (storedUser) {
      try {
        ...
        localStorage.removeItem('snaptour_user');
      }
    }
  }, []);
  ...
  const handleGoogleLogin = () => {
    ...
    const performMockLogin = () => {
       ...
       localStorage.setItem('snaptour_user', JSON.stringify(mockUser));
    };
        ...
        callback: async (tokenResponse: any) => {
          if (tokenResponse && tokenResponse.access_token) {
            try {
              ...
              // Save session
              localStorage.setItem('snaptour_user', JSON.stringify(newUser));
            } catch (error) {
