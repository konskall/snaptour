  ...
  return (
    <div className="relative w-full h-screen overflow-hidden bg-slate-900 text-white" style={backgroundStyle}>
      ...
      <div className={`absolute ${missingCreds.length > 0 ? 'top-10' : 'top-0'} left-0 right-0 p-6 z-40 flex items-center justify-between pointer-events-none transition-all`}>
        <div onClick={resetApp} className="flex items-center gap-2 pointer-events-auto cursor-pointer group">
        ...
        <div className="flex items-center gap-3 pointer-events-auto">
          <div className="relative" ref={langMenuRef}>
            <button onClick={() => setIsLangMenuOpen(!isLangMenuOpen)} className="flex items-center gap-2 bg-black/20 backdrop-blur-md px-3 py-2 rounded-full border border-white/10 hover:bg-white/10 transition-colors shadow-lg">
              <Globe size={18} className="text-indigo-300" />
              <span className="text-sm font-medium hidden sm:inline">{LANGUAGES.find(l => l.code === langCode)?.name}</span>
            </button>
            {isLangMenuOpen && (
              <div className="absolute right-0 top-full mt-2 w-48 bg-slate-800 border border-slate-700 rounded-xl shadow-xl overflow-hidden z-50 animate-fade-in">
                {LANGUAGES.map((lang) => (
                  <button key={lang.code} onClick={() => handleLanguageChange(lang.code)} className={`w-full text-left px-4 py-3 text-sm hover:bg-slate-700 transition-colors flex items-center justify-between ${langCode === lang.code ? 'bg-slate-700/50 text-indigo-400' : 'text-slate-200'}`}>
          ...
          <div className="relative" ref={userMenuRef}>
            {!user ? (
               <button onClick={handleGoogleLogin} className="flex items-center gap-2 bg-white text-slate-900 px-4 py-2 rounded-full font-medium text-sm hover:bg-slate-200 transition-colors shadow-lg border border-slate-300">
                 <svg className="w-4 h-4 flex-shrink-0" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4" /><path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853" /><path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05" /><path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335" /></svg>
                 <span className="hidden sm:inline">{t.login}</span><span className="sm:hidden">Sign in</span>
               </button>
            ) : (
               <>
                <button onClick={() => setIsUserMenuOpen(!isUserMenuOpen)} className="flex items-center gap-2 bg-black/20 backdrop-blur-md px-2 py-2 sm:px-3 sm:py-2 rounded-full border border-white/10 hover:bg-white/10 transition-colors shadow-lg">
                ...
                {isUserMenuOpen && (
                  <div className="absolute right-0 top-full mt-2 w-56 bg-slate-800 border border-slate-700 rounded-xl shadow-xl overflow-hidden z-50 animate-fade-in">
                    ...
                    <button onClick={() => { setState(AppState.VIEWING_HISTORY); setIsUserMenuOpen(false); }} className="w-full text-left px-4 py-3 text-sm hover:bg-slate-700 transition-colors flex items-center gap-3 text-slate-200">
                      <History size={16} className="text-indigo-400" />{t.history}
                    </button>
                    <button onClick={handleLogout} className="w-full text-left px-4 py-3 text-sm hover:bg-slate-700 transition-colors flex items-center gap-3 text-red-300">
      ...
      <div className="relative z-10 w-full h-full flex flex-col">
        {state === AppState.IDLE && <PhotoInput onImageSelect={handleImageSelect} t={t} />}
        {state === AppState.SELECTING_LANDMARK && identificationResult && (
          <LandmarkSelector identification={identificationResult} onSelect={(name) => fetchDetails(name)} onCancel={resetApp} t={t} />
        )}
        {state === AppState.VIEWING_HISTORY && (
          <HistoryView items={userHistory} onClose={() => setState(AppState.IDLE)} onClear={handleClearHistory} onSelect={handleHistorySelect} t={t} />
        )}
        {state === AppState.ANALYZING_IMAGE && <ScanningView imageSrc={selectedImage} t={t} />}
        {state === AppState.FETCHING_DETAILS && <SkeletonCard t={t} landmarkName={identificationResult?.name} />}
        
        {/* Result States including Chat */}
        ...
        {state === AppState.ERROR && (
          <div className="flex flex-col items-center justify-center h-full p-8 text-center animate-fade-in">
            <div className="bg-red-500/20 p-6 rounded-full mb-6 text-red-400"><Zap size={48} /></div>
            <h3 className="text-2xl font-bold mb-2">{t.oops}</h3>
            <p className="text-slate-300 mb-8 max-w-xs mx-auto">{errorMsg}</p>
            <button onClick={resetApp} className="px-6 py-3 bg-white text-slate-900 font-bold rounded-xl hover:bg-slate-200 transition-colors">{t.tryAgain}</button>
          </div>
        )}
      </div>
    </div>
